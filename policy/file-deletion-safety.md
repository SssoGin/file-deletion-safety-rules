...
三种模式以及小范围、中范围和大范围清理都使用同一固定十行、两列 Markdown 表格。请求用户确认时必须直接输出下表作为卡片主体；不得把表格放进代码围栏，不得用逐行冒号列表、散文、项目符号或其他样式代替表格，不得删行、增行、改名或调整顺序。

| 项目 | 内容 |
|---|---|
| 清理对象 | <按用途概括的对象名称> |
| 位置 | <每个顶层文件目标的规范化绝对路径；非文件对象写可准确识别的位置或标识> |
| 清理范围 | <顶层文件、完整目录、Junction 本身或其他顶层对象的数量> |
| 内容 | <普通文件数、普通目录数，或非文件对象的领域事实> |
| 占用空间 | <精确总字节数和易读单位；不以释放空间为目的时明确说明> |
| 清理原因 | <为什么属于本次清理，并注明证据或来源> |
| 清理方式 | <模式、具体动作、可恢复性和是否释放空间> |
| 安全边界 | <Junction、Git 本地内容、受保护内容及明确保留项> |
| 清理结果 | <执行后这些目标和保留对象应处于什么状态> |
| 恢复方式 | <可用的恢复或重新创建方法；不存在时写“无”> |

- `清理对象` 使用用户能判断用途的名称，不得只写“21 项”“缓存文件”等缺少来源或用途的数量标签。
- `位置` 对文件系统目标只列规范化后的顶层目标绝对路径，同一单元格内的多个路径应清晰分隔；后代文件和目录只在 `内容` 中汇总，不逐条展开。每个顶层目标仍须在内部快照中分别记录类型、完整文件数、包含普通目录根的完整目录数和总字节数。
- Git 分支等非文件对象在本规则适用时也使用同一固定十行表格，各单元格按对象的已核验事实填写，不额外规定一套固定子字段。
- `清理范围` 说明本次会处理多少个顶层文件、完整目录、Junction 入口或其他顶层对象；`内容` 对文件系统目标统计普通文件与普通目录，Junction 及其真实目标不得重复计数。
- `占用空间` 同时给出精确总字节数和便于阅读的 KiB、MiB、GiB 或 TiB；模式 A、B 明确写“不释放空间”，模式 C 才能把已确认普通内容的字节数列为预计释放量。
- `清理原因` 必须说明判断依据，例如用户点名、本次测试生成、可重建缓存或已核对的构建产物；Agent 不得仅凭文件名猜测内容无用。
- `安全边界` 只写影响用户决定的边界和保留项；没有异常时简写为“未发现链接、挂载点、Git 本地内容或受保护内容；不处理上述位置以外的路径”。
- `清理结果` 在执行前写预计结果，不能冒充已经完成；执行后必须按第 7 节另报实际结果，并明确成功、失败、跳过和残留。
- `恢复方式` 是固定第 10 行，写明可用的恢复或重新创建方法，例如从隔离目录、回收站、备份恢复，或从已核验的 Git、构建输入重新创建；没有可用方法时写“无”。
- 用户作出决定所需的补充信息必须并入上述对应单元格，不能新增第 11 行。确认指令放在表格之后且不属于卡片，例如“回复‘确认执行’，我只处理表中目标”。
- 不得创建每次清理专用的本地清单文件。不得用链接、折叠区、附件或不透明的快照 ID 代替卡片中用户作决定所需的内容；需要补充时直接在当前对话中展开。
- 会话中断、上下文压缩或切换后，已确认快照无法从当前对话完整恢复时，必须重新扫描、展示新卡并重新确认；不得猜测快照内容或沿用旧确认继续执行。
- 同一批涉及多个本地卷或多个模式时，按卷和模式拆成多张卡；顶层路径过多时继续按用途拆成可读的多张卡。用户看到全部卡后的一次明确确认可以覆盖同一快照，不得因此重复询问。

三种模式在 `清理方式`、`清理结果` 和 `恢复方式` 中至少写明：

- 模式 A：同卷隔离，可恢复，不释放空间；列出计划隔离位置，预计结果是原路径消失且隔离路径存在；恢复方式写从精确隔离路径移回精确原路径。
- 模式 B：自动回收站，可恢复，不释放空间，也不清空回收站；只有 `RECYCLED_VERIFIED` 才能把预计结果报告为实际成功；恢复方式写由用户从当前桌面用户的回收站恢复。
- 模式 C：永久删除，不进入回收站，无法由 Agent 恢复；列出预计释放空间，并明确所有保留目标不变；恢复方式默认写“无”。存在已核验的 Git、备份或可重建来源时，只能把它写成删除后的外部恢复或重建路径，不能把永久删除本身描述为可恢复。

Junction 是指向真实目录的 NTFS 目录入口，不是内容副本。只删除 Junction 本身时，入口路径消失，真实目标保持原样，不会留下一个需要再次清理的独立 Junction 索引文件。Agent 仍须逐个解析并核验每个 Junction，然后在正常卡片的 `安全边界` 中按数量和用途汇总，不展开完整 Junction 对应表：

- Junction 的真实目标也要清理且位于所有已列顶层普通目录之外时，把该真实目标作为独立顶层目标列入 `位置`，纳入完整统计和相同确认；不得因删除入口而默认为真实目标也已清理。
- Junction 的真实目标经规范化和逐级解析后，已经被某个已列顶层目标完整覆盖时，不得重复列入 `位置` 或重复计数；在 `安全边界` 中汇总说明“真实目标已由上述顶层目录覆盖”，并仍按只处理一次的快照验证。
- 共享目标对应的 Junction 只删除 Junction 本身，并在 `安全边界` 和 `清理结果` 中明确真实目标保留。
- 用途不明的 Junction 必须停止，不得展示可执行的确认卡，也不得让用户在缺少用途结论时盲目确认。
- 模式 B 的 helper 会拒绝顶层目标、祖先或后代中的任何 reparse point，因此发现 Junction 时模式 B 不可用，必须在操作前停止，不得展示可执行的模式 B 确认卡。上述“只删除入口”或“真实目标另列”的处理只适用于能够证明操作边界的模式 A 或模式 C。
...
1. 按第 2.2 节展示并确认固定十行两列表格卡，模式 A 的 `清理方式`、`清理结果` 和 `恢复方式` 必须列明精确隔离路径及“可恢复但不释放空间”。
...
- 只允许调用与本策略文件位于同一目录的 `Recycle-Bin-Only.ps1`；helper SHA-256 必须为 `70A676487C3F08480EB4891D09CD85802FD81B06A1B80621D7720B24EAB9212A`。
...
按第 2.2 节展示固定十行两列表格卡，并在对应字段中包含：

- 每个顶层目标的规范化绝对路径，以及汇总后的完整文件数、包含普通目录根的完整目录数和总字节数；
- 工作区根和项目根等 `ProtectedRoot`，以及目标或后代中的 reparse point 处理结论；
- 当前模式为“自动回收站”、可恢复、仍占用原卷空间且不会清空回收站；
- 当前用户/SID/Session/卷组合是否已通过相同 helper 哈希的一次性兼容性测试。未测试时，把同卷唯一小型 fixture 测试写入本次计划；该 fixture 与目标共用这一次确认，不再单独询问；
- 对每个目标先用同一 helper 的 `-ValidateOnly` 结果绑定元数据指纹；该指纹覆盖相对路径、类型，以及文件长度和文件最后写入时间，用于识别数量和总字节数不变的替换、改名或同大小修改；
- 目标卷配置的回收站总容量、当前已用字节数和剩余字节数。目标不能在不超过剩余容量的前提下放入回收站时，本模式不可用，不得冒险淘汰现有回收项目。
...
3. 使用独立子进程和参数数组传入一个精确目标、`ExpectedType`、`ExpectedFileCount`、`ExpectedDirectoryCount`、`ExpectedBytes`、确认前 `-ValidateOnly` 返回的 `ExpectedMetadataFingerprint` 及全部 `ProtectedRoot`；真正执行时缺少或不匹配该指纹必须停止，一次调用只处理一个顶层目标。
4. 同时异步捕获 stdout/stderr，等待真实退出码。禁止在当前 PowerShell 进程中直接调用 helper 后从管道推断结果。
5. helper 必须在操作前验证：Windows 交互会话、当前身份 SID、同 Session Explorer 所有者、固定本地卷、卷级回收站策略、`NukeOnDelete=0`、配置总容量、由 `SHQueryRecycleBinW` 返回的当前占用、剩余容量、目标/祖先/后代 reparse 状态、受保护根，以及类型、数量、字节数和元数据指纹组成的授权快照；调用回收 API 前必须再次验证目标、祖先 reparse 状态和完整快照。当前占用无法查询、目标大于剩余容量或任一快照字段漂移时必须停止。
6. helper 先记录当前 SID 在目标卷上的 `$I` 元数据和 `$R` 数据对象名称集合，再请求 `SendToRecycleBin`；完成后要求原路径不存在、出现唯一的新 `$I` 元数据，并且同后缀 `$R` 数据对象也为本次新增。`$I` 的原始路径、删除时间及文件大小（文件目标）必须匹配，`$R` 的类型、完整文件数、完整目录数、总字节数和元数据指纹也必须与授权快照一致。
...
按第 2.2 节展示固定十行两列表格卡。`位置` 列出每个规范化绝对顶层路径，`清理范围`、`内容` 和 `占用空间` 给出与执行快照一致的类型、完整文件数、目录数和总字节数；`安全边界` 说明目录包含范围及链接、挂载点、Git 本地内容和受保护内容；`清理方式` 必须明确写“永久删除，不进入回收站，无法由 Agent 恢复”，并给出预计释放空间；`恢复方式` 默认写“无”，只有已核验的外部恢复或重建来源才能另行列明。
